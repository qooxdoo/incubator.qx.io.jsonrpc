  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.ui.progressive.renderer.table.Row", {
    ...
    construct: function construct(columnWidths) {
      ...
      this.__columnWidths__P_485_0 = columnWidths; // Create space to store renderers for each column

      this.__renderers__P_485_1 = {}; // We need a default cell renderer to use if none is specified

      this.__defaultCellRenderer__P_485_2 = new qx.ui.progressive.renderer.table.cell.Default(); // We don't yet know who our Progressive will be

      this.__progressive__P_485_3 = null;
      this.__colors__P_485_4 = {};

      this.__linkColors__P_485_5(); // This layout is not connected to a widget but to this class. This class
      ...
      this.__layout__P_485_6 = new qx.ui.layout.HBox();

      this.__layout__P_485_6.connectToWidget(this); // dynamic theme switch


      {
        qx.theme.manager.Meta.getInstance().addListener("changeTheme", this.__linkColors__P_485_5, this);
      }
    },
    statics: {
      /**
    ...
    members: {
      __progressive__P_485_3: null,
      __name__P_485_10: null,
      __hash__P_485_11: null,
      __columnWidths__P_485_0: null,
      __renderers__P_485_1: null,
      __defaultCellRenderer__P_485_2: null,
      __colors__P_485_4: null,
      __layout__P_485_6: null,

      /**
       * Helper to link the theme colors to the current class
       */
      __linkColors__P_485_5: function __linkColors__P_485_5() {
        // link to color theme
        var colorMgr = qx.theme.manager.Color.getInstance();
        this.__colors__P_485_4.bgcol = [];
        this.__colors__P_485_4.bgcol[0] = colorMgr.resolve("progressive-table-row-background-even");
        this.__colors__P_485_4.bgcol[1] = colorMgr.resolve("progressive-table-row-background-odd");
      },
      // overridden
      join: function join(progressive, name) {
        // Are we already joined?
        if (this.__progressive__P_485_3) {
        ...
        this.__progressive__P_485_3 = progressive; // Save the name that Progressive knows us by

        this.__name__P_485_10 = name; // If we haven't created style sheets for this table yet...

        var tr = qx.ui.progressive.renderer.table.Row;

        if (!tr.__clazz__P_485_7) {
          tr.__clazz__P_485_7 = {};
        }

        var hash = progressive.toHashCode();

        if (!tr.__clazz__P_485_7[hash]) {
          // ... then do it now.
          tr.__clazz__P_485_7[hash] = {
            rowstylesheet: null,
            cellstylesheet: []
          };
          var stylesheet = ".qx-progressive-" + hash + "-row {" + "  width : 100%;" + "}";
          tr.__clazz__P_485_7[hash].rowstylesheet = qx.bom.Stylesheet.createElement(stylesheet);

          var columnData = this.__columnWidths__P_485_0.getData();

          for (var i = 0; i < columnData.length; i++) {
            var stylesheet = ".qx-progressive-" + hash + "-col-" + i + " {" + tr.__tableCellStyleSheet__P_485_9 + "}";
            tr.__clazz__P_485_7[hash].cellstylesheet[i] = qx.bom.Stylesheet.createElement(stylesheet);
          } // Save the hash too


          this.__hash__P_485_11 = hash; // Arrange to be called when the window appears or is resized, so we
      ...
      addRenderer: function addRenderer(column, renderer) {
        ...
        this.__renderers__P_485_1[column] = renderer;
      },
      ...
      removeRenderer: function removeRenderer(column) {
        ...
        if (!this.__renderers__P_485_1[column]) {
          throw new Error("No existing renderer for column " + column);
        }

        delete this.__renderers__P_485_1[column];
      },
      // overridden
      render: function render(state, element) {
        ...
        if (state.getRendererData()[this.__name__P_485_10].end === undefined) {
          state.getRendererData()[this.__name__P_485_10] = {
            end: 0,
            start: 1,
            rows: 0,
            totalHeight: 0
        ...
        for (var i = 0; i < data.length; i++) {
          var stylesheet = "qx-progressive-" + this.__hash__P_485_11 + "-col-" + i; // Determine what renderer to use for this column

          renderer = this.__renderers__P_485_1[i] || this.__defaultCellRenderer__P_485_2; // Specify information that cell renderer will need
        ...
        var rendererData = state.getRendererData()[this.__name__P_485_10]; // Track total height so we can determine if there's a vertical scrollbar
        ...
        div.className = "qx-progressive-" + this.__hash__P_485_11 + "-row";
        ...
        switch (element.location) {
          case "end":
            ...
            div.style.backgroundColor = this.__colors__P_485_4.bgcol[index]; // Update state for next time
      ...
      _resizeColumns: function _resizeColumns(e) {
        var pane = this.__progressive__P_485_3.getStructure().getPane();

        var width = pane.getBounds().width - qx.bom.element.Scroll.getScrollbarWidth(); // Get the style sheet rule name for this row

        var stylesheet = ".qx-progressive-" + this.__hash__P_485_11 + "-row"; // Remove the style rule for this row

        var tr = qx.ui.progressive.renderer.table.Row;
        qx.bom.Stylesheet.removeRule(tr.__clazz__P_485_7[this.__hash__P_485_11].rowstylesheet, stylesheet); // Create the new rule for this row

        var rule = "width: " + width + "px;"; // Apply the new rule

        qx.bom.Stylesheet.addRule(tr.__clazz__P_485_7[this.__hash__P_485_11].rowstylesheet, stylesheet, rule); // Compute the column widths

        this.__layout__P_485_6.renderLayout(width, 100, {
          top: 0,
          right: 0,
          bottom: 0,
          left: 0
        ...
        var columnData = this.__columnWidths__P_485_0.getData(); // Reset each of the column style sheets to deal with width changes


        for (var i = 0, left = 0; i < columnData.length; i++, left += width) {
          // Get the style sheet rule name for this cell
          var stylesheet = ".qx-progressive-" + this.__hash__P_485_11 + "-col-" + i; // Remove the style rule for this column

          var tr = qx.ui.progressive.renderer.table.Row;
          qx.bom.Stylesheet.removeRule(tr.__clazz__P_485_7[this.__hash__P_485_11].cellstylesheet[i], stylesheet); // Get this column width.
          ...
          if (qx.core.Environment.get("css.boxmodel") == "content") {
            inset = qx.ui.progressive.renderer.table.Row.__padding__P_485_8 * 2;
          } else {
            inset = -1;
          ...
          var paddingRule = "0px " + qx.ui.progressive.renderer.table.Row.__padding__P_485_8 + "px " + "0px " + qx.ui.progressive.renderer.table.Row.__padding__P_485_8 + "px;";
          var leftRule = left + "px;";
          var rule = tr.__tableCellStyleSheet__P_485_9 + "width: " + widthRule + "left: " + leftRule + "padding: " + paddingRule; // Apply the new rule

          qx.bom.Stylesheet.addRule(tr.__clazz__P_485_7[this.__hash__P_485_11].cellstylesheet[i], stylesheet, rule);
        }
      }
    },
    destruct: function destruct() {
      // remove dynamic theme listener
      qx.theme.manager.Meta.getInstance().removeListener("changeTheme", this.__linkColors__P_485_5, this);
      var name;

      for (name in this.__renderers__P_485_1) {
        this.__renderers__P_485_1[name] = null;
      } // Remove any style sheets that we had added


      var tr = qx.ui.progressive.renderer.table.Row;

      var hash = this.__progressive__P_485_3.toHashCode();

      if (tr.__clazz__P_485_7 && tr.__clazz__P_485_7[hash]) {
        // Remove the row stylesheet
        if (tr.__clazz__P_485_7[hash].rowstylesheet) {
          // Get the style sheet rule name for this row
          var stylesheet = ".qx-progressive-" + this.__hash__P_485_11 + "-row"; // Remove the style rule for this row

          var tr = qx.ui.progressive.renderer.table.Row;
          qx.bom.Stylesheet.removeRule(tr.__clazz__P_485_7[this.__hash__P_485_11].rowstylesheet, stylesheet);
        } // Remove each of the column style sheets


        if (tr.__clazz__P_485_7[hash].cellstylesheet) {
          for (var i = tr.__clazz__P_485_7[hash].cellstylesheet.length - 1; i >= 0; i--) {
            // Get the style sheet rule name for this cell
            var stylesheet = ".qx-progressive-" + this.__hash__P_485_11 + "-col-" + i;
            var rule = tr.__clazz__P_485_7[this.__hash__P_485_11].cellstylesheet[i]; // Remove the style rule for this column
      ...
      if (this.__progressive__P_485_3 && this.__progressive__P_485_3.getRendererData) {
        var rendererData = this.__progressive__P_485_3.getRendererData();

        if (rendererData && rendererData[this.__name__P_485_10] && rendererData[this.__name__P_485_10].end !== undefined) {
          rendererData[this.__name__P_485_10] = null;
        }
      }

      this.__colors__P_485_4 = this.__renderers__P_485_1 = this.__progressive__P_485_3 = this.__columnWidths__P_485_0 = null;

      this._disposeObjects("__layout__P_485_6", "__defaultCellRenderer__P_485_2", "__columnData");
    }
  });
  qx.ui.progressive.renderer.table.Row.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=Row.js.map?dt=1625734533082